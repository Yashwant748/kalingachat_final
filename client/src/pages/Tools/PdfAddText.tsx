...
import { Upload, FileText, Download, AlertCircle, Loader2, Type } from "lucide-react";
...
export default function PdfAddText() {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            toast({ title: "Success!", description: "Text added successfully to the first page." });
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden bg-background">
            <header className="h-16 flex items-center justify-between px-6 border-b border-border/50 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="font-orbitron font-semibold text-lg">Add Text to PDF</h1>
                </div>
            </header>
                ...
                <div className="max-w-xl w-full">
                    <div className="glass-panel p-8 rounded-2xl text-center space-y-8">
                        <div className="space-y-2">
                            <h2 className="text-2xl font-bold">Inject Custom Text</h2>
                            <p className="text-muted-foreground">Stamp custom text at the top of your document offline.</p>
                        </div>

                        <div className="text-left space-y-2">
                            <label className="text-sm font-medium text-foreground ml-1">Text to Add</label>
                            <input 
                                type="text" 
                                value={customText}
                        ...
                        <div
                            className={`border-2 border-dashed rounded-xl p-10 transition-colors ${file ? 'border-indigo-500/50 bg-indigo-500/5' : 'border-border hover:border-primary/50 hover:bg-white/5'}`}
                        ...
                        <div className="space-y-4">
                            ...
                            <Button
                                ...
                                disabled={!file || !customText || isProcessing}
                                onClick={handleProcess}
                            >
                                {isProcessing ? "Processing..." : <><Download className="w-5 h-5 mr-2" /> Add Text & Download</>}
                            </Button>
                        </div>
                    </div>
                </div>
